...
def hentaidl(client, callback_query):
    ...
    if not is_hentai:
        if not url == "":        
            ...
            K = client.send_document(chat_id=chatid, document=f'{link}.mp4', caption=f"""Download By @HentaiR0bot""", parse_mode="markdown")   
            file_id = K.document.file_id
            client.send_document(chat_id=CACHE_CHANNEL, document=file_id, caption=f"""Download By @HentaiR0bot""", parse_mode="markdown")                               
    ...
    if is_hentai:
        if not url == "":        
            ...
            callback_query.edit_message_text("""Uploading Now""", parse_mode="markdown")                       
            client.send_document(chat_id=chatid, document=file_id, caption=f"""Download By @HentaiR0bot""", parse_mode="markdown")                        
        if url == "":     
            file_id = is_hentai["file_id"]  
            callback_query.edit_message_text("""Uploading Now""", parse_mode="markdown")                        
            client.send_document(chat_id=chatid, document=file_id, caption=f"""Download By @HentaiR0bot""", parse_mode="markdown")
